class Bomb:
    ...
    def get_range_chatgpt(self, map):

        self.sectors.append([self.pos_x, self.pos_y])

        for x in range(1, self.range):
            if map[self.pos_x + x][self.pos_y].value == 1:
                break
            elif map[self.pos_x + x][self.pos_y].value == 0 or map[self.pos_x - x][self.pos_y].value == 3:
                self.sectors.append([self.pos_x + x, self.pos_y])
            elif map[self.pos_x + x][self.pos_y].value == 2:
                self.sectors.append([self.pos_x + x, self.pos_y])
                break
        for x in range(1, self.range):
            if map[self.pos_x - x][self.pos_y].value == 1:
                ...
                break
        for x in range(1, self.range):
            if map[self.pos_x][self.pos_y + x].value == 1:
                break
            elif map[self.pos_x][self.pos_y + x].value == 0 or map[self.pos_x][self.pos_y + x].value == 3:
                self.sectors.append([self.pos_x, self.pos_y + x])
            elif map[self.pos_x][self.pos_y + x].value == 2:
                self.sectors.append([self.pos_x, self.pos_y + x])
                break
        for x in range(1, self.range):
            if map[self.pos_x][self.pos_y - x].value == 1:
