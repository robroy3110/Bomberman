...
class Player:
    ...
    def move(self, dx, dy, grid, enemys, power_ups):
        ...
        # right
        if dx == 1:
            if map[tempx+1][tempy] == 0 or map[tempx+1][tempy] == 4:
                self.pos_x += 1
        # left
        elif dx == -1:
        ...
        # bottom
        if dy == 1:
            if map[tempx][tempy+1] == 0 or map[tempx][tempy+1] == 4:
                self.pos_y += 1
        # top
        elif dy == -1:
        ...
        for pu in power_ups:
            if pu.pos_x == math.ceil(self.pos_x / Player.TILE_SIZE) \
                ...
                map[int(pu.pos_x / Player.TILE_SIZE)][int(pu.pos_y / Player.TILE_SIZE)] = 0
